...
const SubmitApplication = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const [applicationAcc, _w] = await getApplicationAccount(program.provider.wallet.publicKey, new PublicKey(grantPublicKey), program.programId)
    const numMilestonesCount = parseInt(milestonesCount)
    const applicationPublicKey = await program.rpc.submitApplication(metadataHash, numMilestonesCount, {
      accounts: {
    ...
    console.log("done");
    console.log(applicationPublicKey)
  };
  return (
    <form
      onSubmit={(e) => {
